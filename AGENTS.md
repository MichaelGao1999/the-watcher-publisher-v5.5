...
### 🧠 认知提取层（用户确认存档后、标准存档动作前执行）

> 本层与标准存档**独立**。标准存档沉淀的是项目级知识（decisions/lessons-learned/troubleshooting），本层沉淀的是**对用户个人有认知提升价值**的内容，发送到 Obsidian 知识库。

**提炼标准**：
- ✅ 可提炼：概念顿悟、决策逻辑、思维模型、踩坑教训、工具心智
- ❌ 不提炼：具体错误修复步骤（→ troubleshooting.md）、项目架构决策（→ decisions.md）、工程经验方法（→ lessons-learned.md）、纯操作流水

> **一句话判断**：三个月后不碰这个项目了，再看这条还能让你「啊对，当时就是这么理解的」——就值得记。

**执行步骤**：
1. 回顾本轮会话，从认知角度提炼 0-5 条候选条目
2. 如无候选 → 输出「本轮无新增认知内容」，直接进入标准存档动作
3. 如有候选 → 以清单形式输出：
...
4. 用户勾选后，检查环境变量 `OBSIDIAN_NTFY_KEY`：
   - 不存在 → 提示用户先配置环境变量（见下方），跳过发送
   - 存在 → 逐条发送
5. 逐条构造 curl，根据当前操作系统选择语法：
   - **Windows**：`curl -d "[%OBSIDIAN_NTFY_KEY%] obsidian [分类]\n标题\n\n正文" ntfy.sh/7q4pizdfds1angdz2qtz`
   - **Mac/Linux**：`curl -d "[$OBSIDIAN_NTFY_KEY] obsidian [分类]\n标题\n\n正文" ntfy.sh/7q4pizdfds1angdz2qtz`
6. 逐条确认 HTTP 200，汇报发送结果
7. 继续执行下方「标准存档动作」

**环境配置**（仅首次，在终端执行）：
- **Windows**：`setx OBSIDIAN_NTFY_KEY qczh4hlim4vs7stn`（关闭终端重开生效）
- **Mac**：`echo 'export OBSIDIAN_NTFY_KEY=qczh4hlim4vs7stn' >> ~/.zshrc && source ~/.zshrc`
